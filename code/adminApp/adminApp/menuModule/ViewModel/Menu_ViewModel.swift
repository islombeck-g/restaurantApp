...
class Menu_ViewModel:ObservableObject{
    @Published var firebase:FireBase_ViewModel
    @Published var name = ""
    @Published var icon = ""
...
//    func checkProducts(productList: [MealProduct])->Bool{
//
//        for i in productList{
//            guard checkProduct(name: i.nameOfProduct, count: i.countOfProduct) else{return false}
//        }
//
//        return true
//
...
//                let index = self.arrayOfMealIn.firstIndex(where: { $0.name == "Some" })
//                print(self.arrayOfMealIn[index!])
